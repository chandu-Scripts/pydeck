...
const barColors = ['#06b6d4', '#8b5cf6', '#ec4899', '#f59e0b']
...
export default function RecallSession() {
  ...
  useEffect(() => {
    if (!user) return
    async function fetchRecallCards() {
      // Get all MCQ cards marked as 'forgot' for this user
      ...
      setQueue(cards || [])
      setInitialCount(cards?.length || 0)
      setLoading(false)
    }
    fetchRecallCards()
  }, [user])
  ...
  useEffect(() => {
    async function fetchResponseStats() {
      if (queue.length === 0 || !queue[currentIndex]) return

      const cardId = queue[currentIndex].id
  ...
  async function handleOptionSelect(optionKey) {
    ...
    const option = optionKey.slice(-1)
  ...
  async function handleSwipe(direction) {
    ...
    if (status === 'mastered') {
      ...
      }).eq('user_id', user.id).eq('flashcard_id', card.id)
      ...
      setStats(prev => ({ ...prev, recall: prev.recall + 1 }))
      ...
      setTimeout(() => {
        ...
        setQueue(prev => {
          const newQueue = [...prev]
          const recalledCard = newQueue.splice(currentIndex, 1)[0]
          newQueue.push(recalledCard)
          return newQueue
        })
        setShowResult(false)
        setSelectedOption(null)
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-5">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">
          {initialCount > 0 ? 'All Caught Up!' : 'No Recall Cards'}
        </h2>
        <p className="text-gray-400 text-center mb-2">
          {initialCount > 0
            ? `You mastered all ${stats.mastered} recall cards`
            : 'You have no cards due for recall'}
        </p>
        <button
          onClick={() => navigate('/analytics')}
          className="mt-4 px-6 py-3 bg-gradient-to-r from-cyan-500 to-cyan-400 text-navy-900 font-semibold rounded-xl cursor-pointer"
        >
          Back to Analytics
        </button>
      </div>
    ...
    <div className="min-h-screen flex flex-col bg-navy-900">
      ...
      <div className="flex items-center justify-between px-5 py-4">
        ...
        <div className="text-center">
          <p className="text-xs text-red-400 font-medium">Recall Mode</p>
        ...
        <div className="flex items-center gap-3 relative">
          ...
          >
            ...
            <span className="text-xs text-cyan-400 font-medium hidden sm:inline">Speed</span>
          </motion.button>
            ...
            {showSpeedPopup && (
              <>
                ...
                <motion.div
                  ...
                  style={{ boxShadow: '0 0 50px rgba(6, 182, 212, 0.4), inset 0 0 30px rgba(6, 182, 212, 0.1)' }}
                ...
                >
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="text-left">
                                ...
                                <p className="text-gray-400 text-xs">{config.duration}s flip</p>
                              </div>
                            </div>
                            {isSelected && (
                              <motion.div
                  ...
                  <p className="text-center text-gray-500 text-xs mt-4">Click outside or select an option to close</p>
                </motion.div>
              </>
            )}
          </AnimatePresence>
      ...
      <div className="px-5 mb-6">
        <div className="h-1 bg-navy-700 rounded-full overflow-hidden">
          <motion.div
            ...
            initial={{ width: 0 }}
            animate={{ width: `${initialCount > 0 ? ((initialCount - queue.length) / initialCount) * 100 : 0}%` }}
        ...
        <div className="w-full max-w-lg">
          ...
          <div style={{ perspective: '2500px' }}>
            ...
            >
              {/* FRONT FACE */}
              <div
                className="absolute inset-0 bg-gradient-to-b from-navy-600/90 to-navy-700/90 backdrop-blur-xl border-2 border-red-500/30 rounded-3xl p-6 flex flex-col shadow-2xl"
                style={{
                  ...
                  {optionKeys.map((key, idx) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className="w-full flex items-center gap-4 p-4 rounded-xl bg-navy-700/50 border border-white/10 hover:border-red-500/50 transition-all cursor-pointer"
                        ...
                        whileTap={buttonTap}
                      >
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm bg-red-500/20 text-red-400">
                          {optionLabels[idx]}
                        </div>
